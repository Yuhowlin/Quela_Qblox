...
def Tomography_GateError_single_shot(QD_agent: QDmanager, ro_elements: list, max_gate_num: int, shots: int = 1000, 
    ...
    pulse_repeats = array(list(range(0, max_gate_num)))
    ...
    folder = {basis: [] for basis in bases}
    if untrained:
    ...
    def repeat_dep_sched(seq_num: int, basis: str):
        slightly_print(f"Shotting for {seq_num} pi-pulses in {basis}-basis")
        sched_kwargs = dict(
            pulse_num=seq_num,
            waveformer=wf_packs,
        ...
        if run:
            ...
            folder[basis].append(reshaped_data)  # (state, ro_q, IQ, shots)
        else:
            pulse_preview(QD_agent.quantum_device, sche_func, sched_kwargs)
    for i in pulse_repeats:
        for basis in bases:
            repeat_dep_sched(i, basis)

    output_dict = {}
    for basis in bases:
        folder[basis] = moveaxis(array(folder[basis]), 0, 2)  # (state,ro_q,IQ,shots) -> (ro_q, IQ, state, shots)
        for q_idx, q_name in enumerate(ro_elements):
            output_dict[f"{q_name}_{basis}"] = (["mixer", "pulse_num", "index"], folder[basis][q_idx])
